...
function yLoop($arr) {
        $.each($arr, function(name, value) {
                //alert(value.content);
                //$($a).append($chatother+value.fromuserid+"&nbsp;"+getLocalTime(value.time)+$chatend+"<br />");
                //$($a).append($chatother+unescape(value.content)+$chatend+"<br />");
                //alert(value.userid);
        });
}
//普通ajax请求
...
function yAjax(ur, ar, fun) {
        ...
        showload();
        ar = makePost(ar);
        $.post(ur, ar, function(data, status) {
                //   alert(fun);
                hideload();
                if (fun == null) {
                        jqformdeal(data);
...
$(function() {
        ...
        $('.navLists li').live('mouseout', function() {
                $(this).children('ul').hide();
        });
        $('.navLists li ul').live('mouseout', function() {
                $(this).hide();
        });
        ////////////////////
...
//格式化输入
//弹出框
//function msgbox(title,obj,width,height,left,top){
//    
//   if(width==undefined){width='';}
//    if(height==undefined){height='';}
//     if(left==undefined){left='calc(25%)';}else{left+='px';}
//      if(top==undefined){top='calc(25%)';}else{top+='px';}
//   if(title!=''){title=title.substr(0,14)}
//   
//      if(boxobj==null){
//      if(typeof(obj)=='object'){
//        //cdebug('1111');
//    var box='<div id="msgbox" class="move"  style="width:'+width+'px;height:'+height+'px;background-color: rgba(0, 0, 0, 0.05);position:fixed;left:'+left+';top:'+top+';padding:8px;overflow: hidden;"><div style="background-color:#7DA6C4" ><div style="float:left;width:100%;text-algin:center;padding: 6px 0;font-weight: bold;"><div style="float:left;width:70%;cursor:move;;margin-left:7px" id="boxtitle"  >'+title+'</div><div style="color:#fafafa;cursor:pointer;float:right;font-size:20px;font-weight:normal;margin-right:4px;width:auto;" id="boxclose" onclick="closebox()">×</div></div><div style="clear:both"></div><div style="padding:7px;background-color:rgb(255, 255, 255);overflow: hidden;" id="boxbody">'+obj.html()+'</div></div></div>';
//    }else{
//       //cdebug('222222222'); 
//    var box='<div id="msgbox" class="move"  style="width:'+width+'px;height:'+height+'px;background-color: rgba(0, 0, 0, 0.05);position:fixed;left:'+left+';top:'+top+';padding:8px;overflow: hidden;"><div style="background-color:#7DA6C4"><div style="float:left;width:100%;text-algin:center;padding: 6px 0;font-weight: bold;"><div style="float:left;width:70%;cursor:move;;margin-left:7px" id="boxtitle" >'+title+'</div><div style="color:#fafafa;cursor:pointer;float:right;font-size:20px;font-weight:normal;margin-right:4px;width:auto;" id="boxclose" onclick="closebox()">×</div></div><div style="clear:both"></div><div style="padding:7px;background-color:rgb(255, 255, 255);overflow: hidden;" id="boxbody">'+obj+'</div></div></div>';  
//        
//    }
//  
//    $('body').append(box);
...
function tools_submit(obj) {
        ...
        if (confirm_flag) {
                ...
                if ($("input[name='" + select_id + "[]']:checked").size() > 0) {
                        ...
                        msgbox('消息提示', "<p class='warning'>没有选择任何项目，无法删除</p>");
                }
        } else {
                //cdebug('form'); 
                form.submit();
        ...
        $('td').click(function() {
                ...
                if ($classname != undefined) {
                        ...
                        case 'ajaxtext':
                                ...
                                break;
                        case 'ajaxchoose':
                                if ($(this).children('div').html() == undefined) {} else {
                                        //cdebug('ggg');
                                        //$(this).html(prevobjval);
                                        //获取子div当前的状态
                                        prevobj = $(this);
