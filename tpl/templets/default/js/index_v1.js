		...
		alert(res.result.length);
            if (res.code == "1") {
                ...
                for (var i = 0; i < res.result.length; i++) {
                    ...
                    //html += '<div class="swiper-slide check_goal_type" data-book_id="` + res.result[i]["book_id"] + `" data-goal_type="` + res.result[i]["goal_type"] + `" data-goal_window="` + res.result[i]["goal_window"] + `" data-isfree="` + res.result[i]["isfree"] + `" data-scan_seat="` + res.result[i]["scan_seat"] + `"><img src="` + res.result[i]["banner_pic"] + `" class="cursor" onerror="this.src=\'images/replace_2.png\'"></div>';
                }
                // 清除
