...
class book extends apiv1base
{
    ...
    public function control_get_section()
    {
        ...
        $data = M('content', 'im')->book_section($this->get_userid(), $book_id['book_id']);
        $this->returnSuccess($data);
    }
    // 获取小说详情
    ...
    //小说漫画获取章节内容
    public function control_get_content()
    {
        M('version', 'im')->lockold($this->head['version']);
        $get = get(['int' => ['book_id' => 1, 'index' => 1, "type" => 1]]);
        if (!$get['type'])
            Out::jerror("数据类型错误", "", "21110");
        $book_id = $get['book_id'];
        $list_order = $get['index'];
        $bname = "";
        $bid = "";
        switch ($get['type']) {
            case '1':
        ...
        $lang = T($bname)->set_where([$bid => $book_id,])->set_field('lang')->get_one();
        $tpsec = M('book', 'im')->gettpsec($get['type'], $lang['lang']);
        $tpsecc = M('book', 'im')->gettpseccontent($get['type'], $lang['lang']);
        $cache = Y::$cache->get($index);
        if ($cache[0]) {
            $arr = $cache[1];
            if (!$arr['next']) {
                $up = T($tpsec)->set_field('list_order')->set_where([$bid => $get['book_id'], 'isdelete' => 0, 'status' => 1])->set_where('list_order>' . $arr['list_order'])->order_by(['f' => 'list_order', 's' => 'up'])->get_one();
                $arr['next'] = $up['list_order'] ? $up['list_order'] : '0';
            }
        } else {
            //M('census', 'im')->uprackreadtime($this->get_userid(), $book_id, 1, $section_id); //更新书架记录
            $data = T($tpsec)->field($f1)->where($where)->where(['status' => 1])->where(['isdelete' => 0])->find();
            if (!$data) {
                Out::jerror('小说或章节不存在', null, '100154');
            }
