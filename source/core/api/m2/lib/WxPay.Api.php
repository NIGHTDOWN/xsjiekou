...
class WxPayApi extends payapi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			...
			$inputObj->SetNotify_url(WXNOTIFY_URL);//异步通知url
		}
		...
		// $inputObj->SetSpbill_create_ip('58.62.92.247');  	    
	...
	{
		//如果不需要上报数据
		if(WXREPORT_LEVENL == 0){
			return;
		} 
		//如果仅失败上报
		if(WXREPORT_LEVENL == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if(WXCURL_PROXY_HOST != "0.0.0.0" 
			&& WXCURL_PROXY_PORT != 0){
			curl_setopt($ch,CURLOPT_PROXY, WXCURL_PROXY_HOST);
			curl_setopt($ch,CURLOPT_PROXYPORT, WXCURL_PROXY_PORT);
		}
