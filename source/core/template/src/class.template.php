...
class Template_Lite
{
  ...
  var $force_compile = G_COMPILE_TPL;  // force a compile regardless of saved state
  ...
  {
  	$out=$this->fetch($file, $cache_id, true);
  ...
  function _fetch_compile($file)
  {
    ...
    try {
     ...
     $file = $this->_file."(".$this->compile_dir.'c_'.$name.")";
     $line = $e->getline();
     $msg  = $e->getMessage();
     $msg  = "tplfille:[$file]line[$line]($msg)";
      error($msg);
    }
    $output = ob_get_contents();
    @ob_end_clean();
  ...
  function trigger_error($error_msg, $error_type = E_USER_ERROR, $file = null, $line = null)
  {
    ...
    error('TPL: [in ' . $this->_file . ' line ' . $this->_linenum . "]: syntax error: $error_msg$info", $error_type);
  }
}
?>
