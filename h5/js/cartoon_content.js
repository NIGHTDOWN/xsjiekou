...
function check_section_id() {
    if (section_id) {
        ...
        // console.log("没有章节参数");
        // 历史阅读章数
        var keeping_track = localStorage.getItem('cartoon' + cartoon);
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (res.code == "1") {
                ...
                console.log('获取详情 cartoon/get_cartoonDetail 错误');
            }
            check_load();
...
function get_section() {
    ...
    $.ajax({
        ...
        success: function(data) {
            console.log("获取列表信息 cartoon/get_cart_section");
            console.log(data);
            var code = data.code;
            // code40001(code);
            ...
            if (code == "1") {
                ...
                if (!section_id) {
                    ...
                    section_id = list[0]["cart_section_id"];
                    console.log("第一次阅读,设置section_id： " + section_id + " 为历史阅读章节")
                }
                // 设置历史阅读章节
                localStorage.setItem('cartoon_id' + cartoon_id, section_id);
                var the_url = window_url + 'cartoon_content.html?cartoon_id=' + cartoon_id + '&section_id=' + section_id;
                history.replaceState('', '', the_url);
                // 判断是否有下标
                if (!index_id) {
                    for (var i = 0; i < list.length; i++) {
                        if (section_id == list[i]["cart_section_id"]) {
                            index_id = i;
                            console.log(list[i]);
                        }
                    }
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    html += `<li class="list_ cursor" data-bid="` + result[i]["book_id"] + `" data-section_id=` + result[i]["cart_section_id"] + ` id="md` + result[i]["cart_section_id"] + `">
                ...
                // 需要付费的金额
                coin = list[index_id]["charge_coin"];
                // 是否免费
        ...
        error: function(e) {
            console.log("获取列表信息 cartoon/get_cart_section 失败");
            check_load();
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (code == "1") {
                ...
                console.log('解锁文章接口 user/deblocking 失败');
                check_code(res.code)
            }
        },
        error: function(e) {
...
$('#next_page').on('click', function() {
    console.log(1);
    if ($(this).is('.no_next')) {
        location.href = 'no_next.html?cartoon_id=' + cartoon_id;
    } else if ($(this).is('.main_button_on')) {
        index_id = parseInt(index_id) + 1;
        section_id = list[index_id]["cart_section_id"];
