...
function get_book_content() {
    ...
    ios_section_id = section_arr[now_number];
...
//没有该书籍
function no_have() {
    $('.fiction_detail').remove();
    var html = `<div class="no_have">
                <img src="/tpl/templets/default/images/jiazai_fair@2x.png">
                <p>ขออภัยที่หาข้อมูลนิยายไม่เจอ</p>
                <button>ไปเขตนิยาย</button>
    </div>`;
    $('.content').after(html);
    $('.content').remove();

    $('.no_have button').on('click', function() {
...
function error_data() {
    ...
    var html = `<div class="no_have">
                ...
                <button>ดาวน์โหลดอีกครั้ง</button>
    </div>`;
    $('.content').after(html);
    $('.content').remove();
